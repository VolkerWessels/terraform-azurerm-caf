...
resource "azurerm_frontdoor_custom_https_configuration" "frontdoor" {
  ...
  dynamic "custom_https_configuration" {
    ...
    content {
      certificate_source = try(each.value.certificate_source, "FrontDoor")
      azure_key_vault_certificate_vault_id = try(each.value.certificate_source, "FrontDoor") != "FrontDoor" && can(custom_https_configuration.value.azure_key_vault_certificate_vault_id) ? try(custom_https_configuration.value.azure_key_vault_certificate_vault_id, try(var.keyvault_certificate_requests[var.client_config.landingzone_key][custom_https_configuration.value.certificate.key].keyvault_id, var.keyvault_certificate_requests[custom_https_configuration.value.certificate.lz_key][custom_https_configuration.value.certificate.key].keyvault_id), null): null
      azure_key_vault_certificate_secret_name = try(each.value.certificate_source, "FrontDoor") != "FrontDoor" && can(custom_https_configuration.value.azure_key_vault_certificate_secret_name) ? try(custom_https_configuration.value.azure_key_vault_certificate_secret_name, try(var.keyvault_certificate_requests[var.client_config.landingzone_key][custom_https_configuration.value.certificate.key].name, var.keyvault_certificate_requests[custom_https_configuration.value.certificate.lz_key][custom_https_configuration.value.certificate.key].name), null): null
      azure_key_vault_certificate_secret_version = try(each.value.certificate_source, "FrontDoor") != "FrontDoor" && can(custom_https_configuration.value.azure_key_vault_certificate_secret_version ? try(custom_https_configuration.value.azure_key_vault_certificate_secret_name, try(var.keyvault_certificate_requests[var.client_config.landingzone_key][custom_https_configuration.value.certificate.key].version, var.keyvault_certificate_requests[custom_https_configuration.value.certificate.lz_key][custom_https_configuration.value.certificate.key].version), null): null
    }
  }
}
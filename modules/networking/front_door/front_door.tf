...
resource "azurerm_frontdoor_custom_https_configuration" "frontdoor" {
  ...
  dynamic "custom_https_configuration" {
    for_each = try(each.value.custom_https_configuration, null) == null ? [] : [lower(each.value.custom_https_configuration.certificate_source) != "frontdoor"  ? each.value.custom_https_configuration : { certificate_source: "FrontDoor"} ]
    content {
      certificate_source = try(each.value.certificate_source, "FrontDoor")
      azure_key_vault_certificate_vault_id = try(custom_https_configuration.value.azure_key_vault_certificate_vault_id, null) == null ? try(try(var.keyvault_certificate_requests[var.client_config.landingzone_key][custom_https_configuration.value.certificate.key].keyvault_id, var.keyvault_certificate_requests[custom_https_configuration.value.certificate.lz_key][custom_https_configuration.value.certificate.key].keyvault_id), null) : custom_https_configuration.value.azure_key_vault_certificate_vault_id
      azure_key_vault_certificate_secret_name = try(custom_https_configuration.value.azure_key_vault_certificate_secret_name, null) == null ? try(try(var.keyvault_certificate_requests[var.client_config.landingzone_key][custom_https_configuration.value.certificate.key].name, var.keyvault_certificate_requests[custom_https_configuration.value.certificate.lz_key][custom_https_configuration.value.certificate.key].name), null) : custom_https_configuration.value.azure_key_vault_certificate_secret_name
